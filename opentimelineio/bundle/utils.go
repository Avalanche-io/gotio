...
type MediaManifest map[string][]*opentimelineio.ExternalReference
...
func PrepareForBundle(
...
) (*opentimelineio.Timeline, MediaManifest, error) {
	...
	for _, clip := range clips {
		...
		if policy == AllMissing {
			replaceMissingRef := opentimelineio.NewMissingReference(
				ref.Name(),
				ref.AvailableRange(),
...
func ConvertToAbsolutePaths(timeline *opentimelineio.Timeline, bundleRoot string) error {
...
func getTargetURL(ref opentimelineio.MediaReference) string {
...
func replaceMissing(clip *opentimelineio.Clip, original opentimelineio.MediaReference, reason string) {
	missing := opentimelineio.NewMissingReference(
		original.Name(),
